...
fn main() -> anyhow::Result<()> {
    ...
    populate_cpuid_page(&vcpu, hva, CPUID_MEMORY_SIZE)?;
    map_memory(
        &vm,
        &snp,
        1,
    ...
    //println!("Guest status: {:?}", status);

    // Run the loop
    for _ in 0..10 {
        let ret: Result<VcpuExit, kvm_ioctls::Error> = vcpu.run();
        eprintln!("cpu exit: {:?}", ret);
        match ret? {
